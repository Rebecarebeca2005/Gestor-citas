...
<div id="modalCita" class="modal hidden">
    <div class="modal-content">
        ...
        <form id="formCita" class="formulario" method="POST" action="index.php?pagina=calendarioAñadir">
            <!-- PASO 1: SERVICIO -->
            ...
            <fieldset class="seccion">
                ...
                <select name="id_disponibilidad">
                    ...
                    $disponibilidad = $citaController->obtenerDisponibilidad($fechaSeleccionada);

                    foreach ($disponibilidad as $d): ?>
                        <option value="<?= $d['id_disponibilidad'] ?>">
                            <?= substr($d['hora_inicio'], 0, 5) ?> - <?= substr($d['hora_fin'], 0, 5) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                 <button type="button" class="btn-atras">
